...
import videoFramesToCanvasArray from './helpers/videoFramesToCanvasArray';
import type { RootState } from 'app/rootReducer';

import type { ScrubberState } from './scrubberSlice';
...
const ScrubberFrames: React.FC<ScrubberFramesProps> = ({
  ...
  const { currentScrubberFrame, scrubberFramesMax } = useSelector<
    RootState,
    ScrubberState
  ...
  const currentTimes = useMemo<Array<number>>(() => {
    ...
    for (let i = 0; i < currentTimes.length; i++)
      currentTimes[i] = i * timeIncrement + 0.1;
    return currentTimes;
  ...
  useEffect(() => {
    videoFramesToCanvasArray(videoSrc, currentTimes, width, canvasWidth).then(
      setCanvasFrames
    );
